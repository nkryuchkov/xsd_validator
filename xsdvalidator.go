...
func directoryAt(path string) directory {
	...
	filepath.Walk(path, func(subpath string, info os.FileInfo, err error) error {
		if err == nil && path != subpath {
			if info.IsDir() {
				dir.subdirs = append(dir.subdirs, subpath)
				return filepath.SkipDir
			}
		}
		return nil
...
func appMain(driver gxui.Driver) {
	...
	window := theme.CreateWindow(800, 600, "Select XML file")
	...
	var selectedXML, selectedXSD string

	open := theme.CreateButton()
	open.SetText("Open")
	open.OnClick(func(gxui.MouseEvent) {
		if window.Title() == "Select XML file" {
			selectedXML = files.Selected().(string)
			window.SetTitle("Select XSD file")
		} else {
			selectedXSD = files.Selected().(string)
			var result string
			if validateXSD(selectedXML, selectedXSD) {
				result = "XML valid as per XSD"
			} else {
				result = "XML not valid as per XSD"
			}
			...
			littleWindow := theme.CreateWindow(150, 30, "XML Validation")
			littleWindow.AddChild(layout)
		}
	})
...
func validateXSD(xmlfile string, xsdfile string) bool {
	xmlbuf, err := ioutil.ReadFile(xmlfile)
	if err != nil {
		log.Printf("%v", err)
		return false
	}
	xsdbuf, err := ioutil.ReadFile(xsdfile)
	if err != nil {
		log.Printf("%v", err)
		return false
	}

	xsdSchema, err := xsd.ParseSchema(xsdbuf)
	if err != nil {
		log.Printf("%v", err)
		return false
	}

	doc := golibxml.ParseDoc(string(xmlbuf))
	if doc == nil {
		log.Printf("Error parsing document")
		return false
	}
	defer doc.Free()
	...
	log.Printf("XML Valid as per XSD")
	return true
}
